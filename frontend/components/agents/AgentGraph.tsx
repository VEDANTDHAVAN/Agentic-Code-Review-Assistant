  ...
  const nodes: Node[] = AGENT_PIPELINE.map((agent, index) => {
    ...
    return {
      ...
      style: {
        ...
        height: 56,
        borderRadius: 8,
        border: active ? "2px solid var(--primary)" : error ? "1px solid var(--danger)" : done ? "1px solid var(--success)" : "1px solid var(--border)",
        background: active ? "color-mix(in srgb, var(--primary) 16%, var(--panel))" : error ? "color-mix(in srgb, var(--danger) 12%, var(--panel))" : done ? "color-mix(in srgb, var(--success) 12%, var(--panel))" : "var(--panel)",
