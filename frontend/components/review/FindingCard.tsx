...
export function FindingCard({ finding, selected, onSelect, onApprove }: FindingCardProps) {
  ...
  return (
    <article className={`rounded-lg border bg-panel-strong p-3 transition ${selected ? "border-primary/70 shadow-lg shadow-cyan-950/20" : "border-border hover:border-primary/40"}`}>
      ...
      <div className="mt-4 flex flex-wrap justify-end gap-2">
        <button className={`inline-flex h-8 items-center gap-2 rounded-md border px-2.5 text-xs font-semibold ${finding.approved ? "border-emerald-400/40 bg-emerald-400/15 text-emerald-700 dark:text-emerald-100" : "border-border bg-panel text-foreground hover:border-primary/50"}`} onClick={() => onApprove(finding.id, true)} disabled={finding.posted}>
          <Check className="h-4 w-4" />
          Approve
        </button>
        <button className="inline-flex h-8 items-center gap-2 rounded-md border border-border bg-panel px-2.5 text-xs font-semibold text-foreground hover:border-primary/50 disabled:opacity-50" onClick={() => onApprove(finding.id, false)} disabled={finding.posted}>
          <X className="h-4 w-4" />
          Reject
        </button>
